...
- **Smart Assistant:** Ready for local LLM (Ollama) or cloud (OpenAI) integrations.
- **Command System:** Natural language parsing combined with explicit commands (`todo`, `git`, `status`, `clear`).
...
- `status` - Print a quick system status overview.
- `todo [list|add <item>|clear]` - Manage your tasks.
- `git` - Check the status of the current git repository.
- `clear` (or `Ctrl+L`) - Clear the interface.
